    ...
    with mlflow.start_run(run_name=f"STGCN_Prod_Train_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}"):
        ...
        for epoch in range(1, EPOCHS + 1):
            ...
            if test_mae_kmh < best_test_mae:
                ...
                logger.info(
                    f"🏆 New best model and scaler saved at epoch {epoch:02d} with Test MAE: {best_test_mae:.4f} km/h."
                )
            else:
                patience_counter += 1
                if patience_counter >= patience:
                    logger.info(
                        f"🛑 Early stopping triggered at epoch {epoch:02d}. Best Test MAE: {best_test_mae:.4f} km/h at epoch {best_epoch:02d}."
                    )
                    break
        ...
        if best_model_state is not None:
            ...
            os.makedirs("models", exist_ok=True)
            torch.save(model.state_dict(), "models/stgcn_prod_latest.pt")
            logger.info("💾 Saved last epoch model weights as fallback.")
        ...
        ax1_sub.plot(
            bin_labels, count_by_bin, color=color, marker="o", linestyle=":", linewidth=2, label="Volume d'exemples"
        )
        ...
        axs[1].plot(
            bin_labels, bias_by_bin, marker="s", color="red", linewidth=2.5, label="Biais systématique (Prédit - Réel)"
        )
        ...
        axs[1].fill_between(
            ...
            bias_by_bin_arr,
            0,
            where=bias_by_bin_arr > 0,
            color="orange",
            alpha=0.3,
            interpolate=True,
            label="Sur-évaluation (vitesse sur-estimée)",
        )
        axs[1].fill_between(
            bin_labels,
            bias_by_bin_arr,
            ...
            alpha=0.15,
            interpolate=True,
            label="Sous-évaluation (vitesse sous-estimée)",
        )
        ...
        axs[2].plot(
            ...
            color="purple",
            linewidth=2.5,
            label="Écart-type des Vitesses Prédites (Dispersion)",
        )
        axs[2].plot(
            bin_labels,
            std_errors_by_bin,
            ...
            linewidth=2.5,
            linestyle="--",
            label="Écart-type des Erreurs (Incertitude des résidus)",
        )
        ...
        axs[3].plot(
            ...
            marker="x",
            linewidth=2,
            label="Vitesse Réelle Cible (Milieu de classe)",
        )
