...
def run_prediction():
    ...
    if os.path.exists(SCALER_PATH):
        ...
        logger.warning(f"⚠️ Scaler introuvable à l'emplacement : {SCALER_PATH}. Ajustement temporaire sur l'échantillon courant.")
    ...
    if os.path.exists(MODEL_PATH):
        ...
        logger.error(f"❌ Modèle introuvable à l'emplacement : {MODEL_PATH}. Un entraînement est nécessaire au préalable.")
        sys.exit(1)
    ...
    for node_idx in range(num_nodes):
        ...
        for h_idx, h in enumerate(HORIZONS):
            ...
            prediction_records.append({
                ...
                "real_speed": round(last_real_speed, 2), # vitesse à t_0 utilisée comme base réelle d'évaluation immédiate
    ...
    sample_df = df_preds.head(10)[["prediction_timestamp", "target_timestamp", "horizon_minutes", "properties_twgid", "predicted_speed", "real_speed"]]
