...
try:
    ...
    logger.warning(
        "⚠️ Impossible d'importer 'get_best_params.py' directement. Assurez-vous d'exécuter le script depuis la racine du projet."
    )
...
DEFAULT_METRICS_PATH = os.path.join(
    ...
    "data",
    "out",
    "monitoring_metrics_morning.json",
)
...
def main():
    ...
    parser.add_argument(
        "--force", action="store_true", help="Force le déclenchement du réentraînement sans vérifier les seuils."
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    if metrics_data:
        ...
        if p_value is not None:
            ...
            if p_value < args.p_value_threshold:
                trigger_needed = True
                reasons.append(
                    f"Dérive de données statistiquement significative (p-value {p_value:.6f} < {args.p_value_threshold})"
                )
        else:
    ...
    if args.dry_run:
        ...
        try:
            ...
            result = subprocess.run(
                [sys.executable, train_script], env=env, check=True, stdout=sys.stdout, stderr=sys.stderr
            )
            print("\n" + "═" * 70)
