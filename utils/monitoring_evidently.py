...
if not DB_PASSWORD:
    logger.error("❌ La variable d'environnement POSTGRES_PASSWORD est manquante ou vide. Impossible de démarrer le monitoring.")
    sys.exit(1)
...
def generate_report():
    ...
    if len(available_days) < 2:
        logger.warning("⚠️ Nombre de jours avec données du matin insuffisant (minimum 2 requis) pour effectuer une comparaison. Monitoring annulé.")
        return
    ...
    df_preds_raw = pd.read_sql(
        text(query_preds), 
        con=engine, 
        params={"day_ref": day_j_minus_1, "day_curr": day_j}
    )
    
    logger.info("📡 Chargement du trafic réel observée depuis PostgreSQL...")
    df_facts_raw = pd.read_sql(
        text(query_facts), 
        con=engine, 
        params={"day_ref": day_j_minus_1, "day_curr": day_j}
    )
    ...
    if df_preds_raw.empty or df_facts_raw.empty:
        logger.warning("⚠️ L'une des tables extraites est vide. Vérifier que l'ingestion et les prédictions ont bien fonctionné.")
        return
    ...
    df_preds_raw["target_dt_min"] = pd.to_datetime(df_preds_raw["target_timestamp"]).dt.tz_localize(None).dt.floor("min")
    ...
    for shift_hours in [0, 1, 2, -1, -2]:
        ...
        df_facts_temp["fact_dt_min"] = (pd.to_datetime(df_facts_temp["timestamp"]).dt.tz_localize(None) + pd.Timedelta(hours=shift_hours)).dt.floor("min")
    ...
    if max_joined_rows <= 0 or best_df_merged is None:
        logger.warning("⚠️ Aucun alignement temporel trouvé après examen de tous les décalages de timezone. Vérifier l'adéquation des horodatages.")
        return
        
    logger.info(f"🏆 Alignement temporel optimal sélectionné : shift de {best_shift:+}h ({max_joined_rows} lignes alignées).")
    ...
    report = Report(metrics=[
        RegressionPreset(),
        ValueDrift(column="target", method="ks", threshold=0.05)
    ])
