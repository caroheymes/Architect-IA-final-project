...
def generate_mapping_exhaustive():
    ...
    with engine.begin() as conn:
        ...
        df_mapping.to_sql(name="ref_segments", con=conn, schema="silver", if_exists="append", index=False)
        ...
        if table_exists:
            ...
            conn.execute(
                text("ALTER TABLE silver.trafic_vitesse_propre ADD COLUMN IF NOT EXISTS properties_gid integer;")
            )
            conn.execute(
                text("ALTER TABLE silver.trafic_vitesse_propre ADD COLUMN IF NOT EXISTS properties_twgid integer;")
            )
