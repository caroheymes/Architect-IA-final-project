        ...
        try:
            ...
            if uncached_idxs:
                ...
                for count in num_pts_per_segment:
                    ...
                    points_per_segment.append([[lon, lat] for lon, lat in zip(lons[start_idx : start_idx + count], lats[start_idx : start_idx + count])])
                    start_idx += count
                ...
                gdf_uncached_wgs84['geometry_wgs84_wkt'] = gdf_uncached_wgs84['geometry_coordinates_obj'].apply(lambda geom: geom.wkt if geom else None)
                gdf_uncached_wgs84['points_json'] = gdf_uncached_wgs84['points'].apply(lambda lst: json.dumps(lst) if lst else None)
                gdf_uncached_wgs84['hexes_json'] = gdf_uncached_wgs84['hexes'].apply(lambda lst: json.dumps(lst) if lst else None)
                gdf_uncached_wgs84['merged_h3_geometry_json'] = gdf_uncached_wgs84['merged_h3_geometry'].apply(lambda geom: json.dumps(geom.__geo_interface__) if geom else None)
