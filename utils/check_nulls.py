...
with engine.connect() as conn:
    ...
    null_gid = conn.execute(text("SELECT count(*) FROM silver.trafic_vitesse_propre WHERE properties_gid IS NULL")).fetchone()[0]
    null_twgid = conn.execute(text("SELECT count(*) FROM silver.trafic_vitesse_propre WHERE properties_twgid IS NULL")).fetchone()[0]
