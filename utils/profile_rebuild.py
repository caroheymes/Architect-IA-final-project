...
def run_profile():
    with engine.begin() as conn:
        snap = conn.execute(text("SELECT id, fetched_at, raw_data FROM bronze.trafic_vitesse_brute ORDER BY fetched_at ASC LIMIT 2;")).fetchall()
    ...
    print(f"3. Cache lookup ({len(trafic)-len(uncached_idxs)} hits, {len(uncached_idxs)} misses): {time.time()-t0:.4f}s", flush=True)
    ...
    df_silver['merged_h3_geometry_json'] = df_silver['merged_h3_geometry'].apply(lambda geom: json.dumps(geom.__geo_interface__) if geom else None)
