...
with engine.connect() as conn:
    ...
    snaps = conn.execute(text("SELECT id, fetched_at, raw_data FROM bronze.trafic_vitesse_brute ORDER BY fetched_at ASC LIMIT 5")).fetchall()
    ...
    for snap_id, fetched_at, raw_data in snaps:
        ...
        if features:
            ...
            print(f"Snap {snap_id} (fetched {fetched_at}): coords_len={len(coords) if coords else 0}, properties.gid={gid}, properties.twgid={twgid}")
        else:
